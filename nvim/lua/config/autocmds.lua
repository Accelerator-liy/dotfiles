...
-- Neovim 0.11 内置 vim 高亮 query 需要较新的 vim parser（含 tab 节点）
-- 旧 parser 会导致按 : 进入命令模式时报 Query err "tab"
	...
	callback = function()
		...
		if not ok then
			vim.schedule(function()
				vim.notify("vim treesitter parser 过期，正在执行 :TSUpdate vim …", vim.log.levels.WARN)
				pcall(vim.cmd, "TSUpdate vim")
			end)
		end
	end,
})
